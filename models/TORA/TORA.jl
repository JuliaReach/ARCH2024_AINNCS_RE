...
using Plots: plot, plot!, lens!, bbox, savefig
...
path = @current_path("TORA", "controllerTora.mat")
...
path = @current_path("TORA", "nn_tora_relu_tanh.mat")
controller_relutanh = read_MAT(path, act_key="act_fcns")
path = @current_path("TORA", "nn_tora_sigmoid.mat")
controller_sigmoid = read_MAT(path, act_key="act_fcns")
...
# To enclose the continuous dynamics, we use a Taylor-model-based algorithm. For
# verification, we also use an additional splitting strategy to increase the
# precision in scenario 1. These algorithsm are defined later for each scenario.
...
function run(; scenario1::Bool, ReLUtanh_activations)
    if scenario1
        ...
        algorithm_plant = TMJets(abstol=3e-2, orderT=3, orderQ=1)
    ...
    print_timed(res)
    print(io, "TORA,$scenario,$result,$(res.time)\n")
...
savefig(fig, joinpath(TARGET_FOLDER, "TORA-ReLU-x1-x2.png"))
...
fig = plot_helper1(vars)
plot!(fig; xlab="x₃", ylab="x₄")
savefig(fig, joinpath(TARGET_FOLDER, "TORA-ReLU-x3-x4.png"))
...
savefig(fig, joinpath(TARGET_FOLDER, "TORA-ReLUtanh.png"))
...
savefig(fig, joinpath(TARGET_FOLDER, "TORA-sigmoid.png"))
