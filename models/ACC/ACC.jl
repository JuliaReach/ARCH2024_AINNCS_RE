...
import MAT, YAML, OrdinaryDiffEq, Plots
...
path = @current_path("ACC", "controller_5_20.mat")
controller_relu = read_MAT(path; act_key="act_fcns")

path = @current_path("ACC", "tanh.yml")
...
function run(; use_relu_controller::Bool)
    ...
    print_timed(res)
    print(io, "ACC,$scenario,$result,$(res.time)\n")
...
savefig(fig, joinpath(TARGET_FOLDER, "ACC-ReLU.png"))

#-

fig = plot_helper(sol_tanh, sim_tanh)
savefig(fig, joinpath(TARGET_FOLDER, "ACC-tanh.png"))
