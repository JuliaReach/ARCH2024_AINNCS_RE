...
path = @current_path("InvertedTwoLinkPendulum",
                     "controller_double_pendulum_less_robust.nnet")
controller_lr = read_NNet(path)

path = @current_path("InvertedTwoLinkPendulum",
                     "controller_double_pendulum_more_robust.nnet")
controller_mr = read_NNet(path);
...
function run(; less_robust_scenario::Bool)
    ...
    print_timed(res)
    print(io, "DoublePendulum,$scenario,$result,$(res.time)\n")
...
savefig(fig, joinpath(TARGET_FOLDER, "InvertedTwoLinkPendulum-less-robust-x1x2.png"))
...
fig = plot_helper(vars, sol_lr, sim_lr, prob_lr, spec_lr)
plot!(fig; xlab="θ₁'", ylab="θ₂'")
savefig(fig, joinpath(TARGET_FOLDER, "InvertedTwoLinkPendulum-less-robust-x3x4.png"))
...
plot!(fig; xlab="θ₁'", ylab="θ₂'")
savefig(fig, joinpath(TARGET_FOLDER, "InvertedTwoLinkPendulum-more-robust.png"))
